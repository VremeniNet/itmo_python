...
def update_currency_rates() -> None:
    ...
    веб-интерфейс (/currency/update), поэтому здесь только начальная загрузка.
    """
...
class MyRequestHandler(BaseHTTPRequestHandler):
    ...
    def _serve_static(self, path: str) -> None:
        """Отдача статичных файлов из папки static."""
    ...
    def do_GET(self) -> None:
        ...
        if path == "/":
            ...
            self.handle_currency_update(parsed)
        elif path == "/currency/delete":
            self.handle_currency_delete(parsed)
        elif path == "/author":
            self.handle_author()
        else:
    ...
    def handle_currencies(self) -> None:
        ...
        Значения value были инициализированы при старте сервера через
        update_currency_rates(), а далее могут изменяться вручную через
        /currency/update.
        """
        context = {
            "app": app_info,
    ...
    def handle_currency_update(self, parsed) -> None:
        """
        Маршрут /currency/update?char_code=USD&value=95.7

        Обновляет курс указанной валюты и перенаправляет обратно на /currencies.
        """
        query = parse_qs(parsed.query)
        char_code = query.get("char_code", [None])[0]
        value_str = query.get("value", [None])[0]

        if not char_code or value_str is None:
        ...
        if new_value is not None and new_value >= 0:
            for cur in currencies:
                if cur.char_code.upper() == char_code.upper():
                    cur.value = new_value
                    break
    ...
    def handle_currency_delete(self, parsed) -> None:
        """
        Маршрут /currency/delete?id=...

        Удаляет валюту из списка и связанных подписок, затем
        перенаправляет на /currencies.
        """
        query = parse_qs(parsed.query)
        raw_id = query.get("id", [None])[0]

        try:
            currency_id = int(raw_id) if raw_id is not None else None
        except ValueError:
            currency_id = None

        if currency_id is not None:
            global currencies, user_currency_relations
            currencies = [c for c in currencies if c.id != currency_id]
            user_currency_relations = [
                rel for rel in user_currency_relations
                if rel.currency.id != currency_id
            ]
